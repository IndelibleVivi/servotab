...
| Website source | `0.5.0` listing-status, method-icon, and motion polish live | Homepage, Docs, Terms, Footer, and Methods catalog link the live listing, separate the newer source candidate, render the 12 canonical leaf glyphs, and add quiet reduced-motion-aware status, icon, and contract-link cues; fresh desktop/mobile live QA is green |
...
| Git publication | `0.5.0` package integrated through PR #20; site polish through PR #22 as `4fbe889` | Immutable package source `05434fa` and artifact receipt `8db4e95` are public ancestors; latest post-merge run `33856781309` is green; tag and GitHub Release remain separate and unperformed |
