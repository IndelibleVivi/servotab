...
| Source candidate | `0.5.0` on local branch `feat/v0.5-skill-icons` | Per-skill icon integration and publication-status reconciliation are source-complete; immutable package source is `05434fa`, while public Git/CI integration remains pending and no Servotab tag or GitHub Release exists |
...
| Git publication | Last integrated `main` remains `82be13e`; `0.5.0` branch is local | Immutable package source commit `05434fa` exists locally; the artifact receipt is being recorded separately, and the branch has not yet been pushed, reviewed by required CI, or merged; tag and GitHub Release remain separate and unperformed |
