...
The implementation is proposed in [draft PR #34](https://github.com/IndelibleVivi/servotab/pull/34), not merged into `main`. Implementation commit `b4f9adf` passed [push Validate](https://github.com/IndelibleVivi/servotab/actions/runs/35497466236) and [PR Validate](https://github.com/IndelibleVivi/servotab/actions/runs/35497497273), including both Ubuntu Python versions, macOS, Windows generated sync, public-tree and site checks. The push run also built and checked release artifacts. A local clean-commit build passed the four-output release check for the same source; these archives remain candidate artifacts, not a published release. Later documentation receipts do not change what those pinned runs proved.
...
The previously recorded public OpenAI directory payload was `0.4.0-rc1`, at the [Servotab listing](https://chatgpt.com/plugins/plugins_6a952d7c729c819196646fda7ec9ad94). PRs #25 and #26 document 0.6 upload-scanner rejections and the resulting dimension fixes. On 2026-09-15, the owner reported that the exact 0.6.2 plugin ZIP was uploaded successfully after confirming the portal's expected portable-manifest normalization notice. This establishes upload into the owner-controlled directory workflow, not submission, review, approval, publication, or an updated public listing. Verify those later states before announcing them.

Previous 0.4/0.5 submission archives and the tagged 0.6.1 assets remain historical artifacts. Use only an archive whose receipt, version, source commit, and manifest match the intended operation.

## Website and infrastructure

The 0.6.2 website was deployed to the existing Cloudflare Pages project from the exact `servotab-site` artifact produced by main Validate run `34984233762` for source `3909242bf142e2b136dde4d96322f4ed649f0164`. Cloudflare recorded a production deployment from branch `main`. Fresh canonical-host checks returned the 0.6.2 status and publisher mapping from `https://servotab.com/`; the docs, privacy, and terms content also matched the 0.6.2 source. HTTP, `www`, and the source-bound Pages deployment URL redirected to the canonical HTTPS host. No DNS, telemetry, account topology, or redirect configuration was changed during this deployment.
