...
GitHub publication does not establish installation, activation, website deployment or live model effectiveness. The public OpenAI directory listing was still observed at 0.6.3 when this release was tagged; the later 0.6.4 directory publication and production website deployment are separate observed states recorded below. No live target-model attempt or improvement claim is established.
...
The 0.6.4 website was deployed on 2026-09-25 to the existing Cloudflare Pages project from the exact `servotab-site` artifact produced by successful main Validate run [`36089194593`](https://github.com/IndelibleVivi/servotab/actions/runs/36089194593) for source `1f074b4ab1448f6a75b8a230d32ae91b9046a171`. Cloudflare deployment `5d4beb98-59cc-4a90-b3a0-5dc70819c2dc` records environment `Production`, branch `main`, and source `1f074b4`; the previous `73791672` deployment remains in Pages history. Fresh canonical-host readback returned `200` for all seven content routes and `404` for a missing route, retained the CSP and security headers, and showed Directory version `0.6.4` on the homepage and Docs page plus the `v0.6.4` source-install command. The homepage and Docs response bodies matched the downloaded CI artifact byte for byte, while the version-specific Pages URL redirected to the canonical domain.

The preceding 0.6.3 website deployment used the exact `servotab-site` artifact produced by successful main Validate run `35538922173` for source `5627bc9b25ed2ede8fcff42393f2e60b2c52a356`. Cloudflare deployment `73791672-e0af-4f87-a2ff-efd3ca087704` recorded environment `Production`, branch `main`, and source `5627bc9`. The related-project source passed the nine website tests, production build, desktop/mobile Docs layout, and no-JavaScript link checks. On 2026-09-21 the canonical Docs URL returned `200`, retained the Google verification tag and CSP, and exposed both related-project links in a live browser.
