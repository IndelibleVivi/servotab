...
Refresh each item's tip, full local/ignored state, index flags and any affected present tracked content, locks and active ownership immediately before its command. If unchanged, execute ordinary `git worktree remove ../experiment` and `git worktree remove ../integrated`. If either refuses, investigate without force. Leave ../changed pending because new notes invalidate its old disposition; do not stash, commit or discard the notes. This does not stop the two independent unchanged items.
