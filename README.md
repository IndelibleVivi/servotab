...
Current source candidate: **0.6.4**, refining cross-boundary experiment selection and evidence scope in `debug` and `verify`. Three diagnostic cases build on the merged discussion-intake rehearsal and bring the candidate source pack to twenty-four. See the [0.6.4 candidate notes](docs/releases/0.6.4.md). This candidate does not establish a tagged release, installation, website deployment, directory update, or improved live model behavior.
