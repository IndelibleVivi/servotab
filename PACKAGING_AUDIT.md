...
- the GitHub repository has been renamed in place to `IndelibleVivi/servotab`; core migration PR #7 merged to public `main` as `f110fbcd` and its post-merge Validate run succeeded, while public-availability copy remains a separate follow-up PR boundary;
- OpenAI directory submission has not been made and remains owner-gated.

A green source gate proves package consistency; the separate maintainer source/cache, fresh-process, and fresh-task receipts prove only the named local installation and behavior boundaries. They do not prove other-machine installation, general behavior quality, the follow-up public-availability copy, final clean-commit deployment, or OpenAI review.
