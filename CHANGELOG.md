...
- Choose responsibility before deep execution in the router, `delegate`, `execute`, and `debug`: allow a bounded unknown-root-cause investigation as one worker lane, keep a responsibility's coupled parts together in a single lane without treating coupling as a reason to stay in the main agent, treat serial delegation as valid while parallelism stays a separate decision, reuse an active worker without duplicating its implementation, forbid recursive delegation, honor explicit solo requests, and sequence work locally when subagent capability is unavailable. Servotab still sets no model, budget, provider, or transport policy, and clear changes stay direct.
...
The exact `v0.6.2` revision and four release assets were published on 2026-09-15; see [release notes](docs/releases/0.6.2.md) and [current state](docs/current-state.md). Later source archives do not replace those immutable artifacts or prove a directory update.
